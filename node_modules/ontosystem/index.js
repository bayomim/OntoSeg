...
exports.run_system = function (filePath, fileName, cb) {
    ...
    // read the text file
    var txtFilePath = "data/text/" + filePath + "/" + fileName + "_vsm.txt"; // read the vsm, the new files
    ...
    buildvsm.build(sentences, 0, function (vsm) {
        ...
        //write the vsm for the first time onle then comment this code
        var vsmFilePath = "data/text/" + filePath + "/" + fileName + "_vsm.txt";
        var vsmTowrite = "";
        for (var i = 0; i < vsm.length; i++) {
            vsmTowrite += vsm[i].toString() + "\n";
        }
...
function writeVSM(_vsm, _vsmFilePath) {
    var vsmTowrite = "";
    for (var i = 0; i < _vsm.length; i++) {
        vsmTowrite += _vsm[i].toString() + "\n";
    }
    
    var file = fs.createWriteStream(_vsmFilePath);
    file.on('error', function (err) { console.log("My_Error:" + err); });
    file.write(vsmTowrite);
    file.end();
    // end of write vsm
}